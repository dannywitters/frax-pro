...
import CurrentMarker from './RevenuesScene/CurrentMarker'
import DataLine from './RevenuesScene/DataLine'
import DataLineEnd from './RevenuesScene/DataLineEnd'
import EthMarker from './RevenuesScene/EthMarker'
import StakingRewardsChartTooltip from './RevenuesScene/StakingRewardsChartTooltip'
...
const RevenuesChart: React.FC<Props> = ({
  style = {},
  data = [],
  dataKeys = { x: 'x', y: 'y' },
  ...
  const tooltipCoords = (_state: NBaseLineChart.State, _current: NBaseLineChart.Datum): NBaseLineChart.Datum =>
    _state.mouse.datum ? _state.mouse.datum : _current

  return (
    <>
      <div style={{ width: '100%', position: 'relative', ...style }}>
        <BaseLineChart margin={_margin} data={_data} {...{ ...props, color, dataKeys }}>
          {({ state }) => {
              ...
              !!(chart.x && chart.y && chart.line) && (
                <>
                  ...
                  {!!xAxis && (
                    <>
                      {/* The x-axis ticks */}
                      <BaseLineChartAxis margin={state.margin} {...{ state, dimensions, dataKeys, ...xAxis }} />
                      {/* The x-axis title */}
                      <text
                  ...
                  {/* The minimum/maximum sides of the curve get a diamond
                      indicator */}
                  <DataLineEnd x={chart.x(data[0][x])} y={chart.y(data[0][y])} {...{ color }} />
                  <DataLineEnd
                  ...
                  {/* The larger diamod indicating where "current" staked is on
                    ...
                    */}
                  <CurrentMarker x={chart.x(current[x])} y={chart.y(current[y])} />
                  ...
                  {showTooltips && (
                    <StakingRewardsChartTooltip
                      ...
                      dimensions={state?.chartRef?.getBoundingClientRect() || {}}
                    />
                  )}
