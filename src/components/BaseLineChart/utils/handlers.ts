...
export const onMouseMove =
  (props: NBaseLineChart.Props, state: NBaseLineChart.State, actions: NBaseLineChart.Actions) =>
  (e: NBaseLineChart.ModifiedMouseEvent<SVGSVGElement>) => {
    ...
    if (leftDatum && rightDatum) {
      ...
      datum = Math.abs(x0! - leftDatum[x]) < Math.abs(x0! - rightDatum[x]) ? leftDatum : rightDatum
    } else if (leftDatum) {
      datum = leftDatum
    } else {
      datum = rightDatum
...
export const onMouseLeave =
  ...
  (e: NBaseLineChart.ModifiedMouseEvent<SVGSVGElement>) => {
    ...
    const outsideChartX = e.clientX < clientRect.x || e.clientX > clientRect.x + clientRect.width
    const outsideChartY = e.clientY < clientRect.y || e.clientY > clientRect.y + clientRect.height
